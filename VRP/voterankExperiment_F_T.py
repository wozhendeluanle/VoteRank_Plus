...
def compute_probability(G):
    ...
    k = k / G.number_of_nodes()  # average degree
...
edgelist = ['Facebook']
# edgelist = ['CEnew', 'jazz', 'router', 'yeast', 'power', 'email', 'netscience', 'USAir2010', 'hamster']
# edgelist = ['condmat', 'Gollowa', 'dblp', 'amazon']
for graph in edgelist:
    ...
    topK = int(0.03 * len(G))  # F_T实验中设置为0.03
    nOfh_index = 1
    order = 2
    lambdaa = 0.1  # 所有实验均设置为0.1
    avg = 100
    ...
    # new_meanresult = get_sir_result(G, new_result, topK, avg, infect_prob, cover_prob, max_iter)
    ...
    df.to_excel(graph+'run'+str(avg)+'times,with proportion'+str(topK)+'.xlsx')
